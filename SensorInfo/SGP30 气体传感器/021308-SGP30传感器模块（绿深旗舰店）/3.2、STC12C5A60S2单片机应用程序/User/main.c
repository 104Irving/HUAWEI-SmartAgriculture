/*
1.项目名称：绿深旗舰店SGP30模块STC12C5A60S2测试程序
2.显示模块：串口返回数据,波特率9600
3.使用软件：keil4 for 51或keil5 for 51
4.配套上位机：无
5.项目组成：SGP30模块,绿深51综合测试板
...
//说明：本例程是基于STC12单片机写的，有些寄存器地址在通用的51头文件如reg51.h或reg52.h并未声明，但是为了程序编写方便
//还是直接调用了普通51单片机的头文件，因此在程序中会看到诸如sfr P4 = 0xC0、sfr P4SW = 0xBB等一些以sfr开头的语句，这
//语句就是为声明STC12单片机独有的寄存器的地址，具体可参考STC12C5A60S2数据手册，以后在使用时不再注释。
